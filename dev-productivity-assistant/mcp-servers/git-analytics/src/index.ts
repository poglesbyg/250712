...
class GitAnalyticsServer {
  ...
  private async analyzeCommitPatterns(args: unknown) {
  ...
  private async analyzeBranchInsights(args: unknown) {
    ...
    try {
      ...
      // Get merge commits
      const mergeCommits = log.all.filter(commit => 
  ...
  private analyzeBranchNaming(branches: string[]) {
    const patterns = {
  ...
  private async findStaleBranches(git: any, branches: string[]) {
