...
class KnowledgeGraphServer {
    ...
    async initializeNeo4j() {
        try {
            // Connect to Neo4j
            this.driver = neo4j.driver('bolt://localhost:7687', neo4j.auth.basic('neo4j', 'devproductivity'));
            // Test connection
    ...
    async extractCodeKnowledge(session, projectPath) {
        ...
        for (const filePath of files) {
            try {
                ...
                // Create file node
                await session.run(`CREATE (f:File {
            path: $path,
            name: $name,
            lines: $lines,
            size: $size
                ...
                for (const entity of codeStructure.entities) {
                    await session.run(`CREATE (e:${entity.type === 'function' ? 'Function' : 'Class'} {
              name: $name,
              file: $file,
             ...
             CREATE (f)-[:CONTAINS]->(e)`, { filePath: relativePath, name: entity.name });
                    relationships++;
                }
                // Create import relationships
                for (const dep of codeStructure.dependencies) {
                    await session.run(`MATCH (f1:File {path: $fromFile})
             MATCH (f2:File {path: $toFile})
             CREATE (f1)-[:IMPORTS]->(f2)`, { fromFile: relativePath, toFile: dep });
                    relationships++;
                }
            }
            catch (error) {
    ...
    async extractGitKnowledge(session, projectPath) {
        ...
        try {
            ...
            for (const author of authors) {
                await session.run(`CREATE (a:Author {
            name: $name,
            email: $email
          })`, {
                    name: author,
            ...
            for (const commit of log.all) {
                await session.run(`CREATE (c:Commit {
            hash: $hash,
            message: $message,
            date: $date,
            ...
            deletions: $deletions
                ...
                await session.run(`MATCH (a:Author {name: $author})
           MATCH (c:Commit {hash: $hash})
           CREATE (a)-[:AUTHORED]->(c)`, { author: commit.author_name, hash: commit.hash });
                relationships++;
                // Create file change relationships
                const files = await git.show(['--pretty=format:', '--name-only', commit.hash]);
                for (const file of files.split('\n').filter(f => f.trim())) {
                    await session.run(`MATCH (c:Commit {hash: $hash})
             MATCH (f:File {path: $file})
             CREATE (c)-[:MODIFIES]->(f)`, { hash: commit.hash, file });
                    relationships++;
                }
            }
            return { entities, relationships };
    ...
    async generateGraphInsights(session) {
        const insights = [];
        try {
    ...
    translateToQuery(naturalLanguageQuery) {
