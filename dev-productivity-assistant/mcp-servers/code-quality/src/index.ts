...
class CodeQualityServer {
  ...
  private async analyzeComplexity(args: unknown) {
    ...
    try {
      ...
      const fileMetrics: Record<string, ComplexityMetrics> = {};
  ...
  private analyzeFileComplexity(filePath: string): ComplexityMetrics {
  ...
  private getComplexityDistribution(fileMetrics: Record<string, ComplexityMetrics>) {
    const distribution = { low: 0, medium: 0, high: 0, veryHigh: 0 };
    
    Object.values(fileMetrics).forEach(metrics => {
      if (metrics.cyclomaticComplexity <= 5) distribution.low++;
      else if (metrics.cyclomaticComplexity <= 10) distribution.medium++;
      else if (metrics.cyclomaticComplexity <= 20) distribution.high++;
  ...
  private getMostComplexFiles(fileMetrics: Record<string, ComplexityMetrics>, limit: number) {
  ...
  private async detectTechDebt(args: unknown) {
  ...
  private detectFileIssues(filePath: string, content: string, metrics: ComplexityMetrics, threshold: number) {
    const issues = [];
  ...
  private categorizeIssues(techDebtFiles: any[]) {
  ...
  private generateRecommendations(techDebtFiles: any[]) {
    const recommendations = [];

    if (techDebtFiles.some(f => f.issues.some((i: any) => i.type === 'high_complexity'))) {
      recommendations.push('Consider breaking down complex functions into smaller, more manageable pieces');
    }

    if (techDebtFiles.some(f => f.issues.some((i: any) => i.type === 'long_file'))) {
      recommendations.push('Split large files into multiple modules with single responsibilities');
    }

    if (techDebtFiles.some(f => f.issues.some((i: any) => i.type === 'high_coupling'))) {
      recommendations.push('Reduce dependencies by implementing dependency injection or facade patterns');
    }

    if (techDebtFiles.some(f => f.issues.some((i: any) => i.type === 'duplicate_code'))) {
  ...
  private async suggestRefactoring(args: unknown) {
  ...
  private estimateRefactoringImpact(suggestions: any[]) {
    const impact = {
      complexity: 0,
      maintainability: 0,
      testability: 0,
