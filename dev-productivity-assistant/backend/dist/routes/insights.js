...
const insightRequestSchema = zod_1.z.object({
    type: zod_1.z.enum(['productivity', 'code_quality', 'patterns', 'recommendations', 'anomalies']),
...
const codeAnalysisSchema = zod_1.z.object({
    ...
    projectPath: zod_1.z.string().optional(),
    analysisType: zod_1.z.enum(['complexity', 'patterns', 'smells', 'security', 'performance']).optional()
});
...
router.get('/anomalies', async (req, res) => {
    try {
        ...
        const repoPath = projectPath || process.cwd();
        const analyzeDays = parseInt(days) || 30;
...
async function generateInsight(type, projectPath, startDate, endDate, context) {
    try {
        let analysisData = {};
...
async function gatherProductivityData(projectPath, startDate, endDate) {
...
async function gatherCodeQualityData(projectPath) {
    try {
...
async function gatherPatternData(projectPath, startDate, endDate) {
    const git = (0, simple_git_1.default)(projectPath);
    try {
        const log = await git.log({
            from: startDate.toISOString(),
...
async function gatherRecommendationData(projectPath) {
    try {
...
async function gatherAnomalyData(projectPath, startDate, endDate) {
    const git = (0, simple_git_1.default)(projectPath);
    try {
        const log = await git.log({
            from: startDate.toISOString(),
...
async function generateAIAnalysis(type, data, context) {
...
function buildAnalysisPrompt(type, data, context) {
...
function extractActionItems(text, type) {
    const actionItems = [];
    const lines = text.split('\n');
    for (const line of lines) {
...
function generateFallbackAnalysis(type, data) {
    ...
    return fallbackAnalyses[type] || fallbackAnalyses.productivity;
}
async function analyzeCodeWithAI(projectPath, files, analysisType = 'complexity') {
    try {
        const targetFiles = files || await (0, glob_1.glob)('**/*.{js,ts,jsx,tsx}', {
            cwd: projectPath,
            ignore: ['node_modules/**', 'dist/**', 'build/**', '.git/**']
        });
        const results = [];
...
async function analyzeFileWithAI(filePath, content, analysisType) {
...
async function generateProductivityRecommendations(projectPath) {
    try {
        ...
        return {
            ...
            priority: 'high',
            categories: {
                codeQuality: aiAnalysis.recommendations.filter((r) => r.includes('code') || r.includes('quality') || r.includes('test')),
                productivity: aiAnalysis.recommendations.filter((r) => r.includes('productivity') || r.includes('efficiency') || r.includes('workflow')),
                collaboration: aiAnalysis.recommendations.filter((r) => r.includes('team') || r.includes('collaboration') || r.includes('review'))
            }
        };
    }
    catch (error) {
...
async function detectAnomalies(projectPath, days) {
    try {
...
async function generateLearningInsights(projectPath) {
    try {
...
async function analyzeFileChangePatterns(git, startDate, endDate) {
    try {
        const log = await git.log({
            from: startDate.toISOString(),
            to: endDate.toISOString(),
...
function analyzeWorkingPatterns(commits) {
...
function analyzeCollaborationPatterns(commits) {
...
function analyzeCommitSizes(commits) {
...
function analyzeAuthorActivity(commits) {
    const authors = new Map();
    commits.forEach(commit => {
        const author = commit.author_name;
