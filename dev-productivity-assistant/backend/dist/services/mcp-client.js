...
class MCPServerProcess extends events_1.EventEmitter {
    constructor(server) {
        super();
        this.server = server;
        this.process = null;
...
class MCPClient {
    ...
    simulateKnowledgeGraph(toolName, parameters) {
        switch (toolName) {
    ...
    simulateServerStart(serverName) {
        const server = this.servers.get(serverName);
        if (!server) {
            return false;
        }
