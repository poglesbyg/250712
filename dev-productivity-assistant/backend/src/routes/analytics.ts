...
interface ProductivityMetrics {
  ...
  focusBlocks: Array<{ start: string; end: string; duration: number }>;
...
function analyzeCommitFrequency(commits: any[], startDate: Date, endDate: Date) {
...
function analyzePeakHours(commits: any[]) {
...
async function analyzeCodeQuality(projectPath: string) {
  try {
...
async function analyzeLanguageBreakdown(projectPath: string) {
  try {
...
function calculateProductivityScore(
  ...
  if (codeQuality.complexity < 10) score += 20;
  else if (codeQuality.complexity < 15) score += 15;
  else if (codeQuality.complexity < 20) score += 10;
  else score += 5;
...
function calculateTotalCodingTime(commits: any[]): number {
...
function generateFocusBlocks(commits: any[]) {
...
function calculateContextSwitches(commits: any[]): number {
...
async function analyzeFocusPatterns(startDate: Date, endDate: Date) {
...
async function analyzeContextSwitches(startDate: Date, endDate: Date) {
...
async function analyzeCodingTime(projectPath: string, startDate: Date, endDate: Date) {
  const git = simpleGit(projectPath);
  ...
  try {
    ...
    const dailyCodingTime = new Map<string, number>();
...
async function analyzeProductivityTrends(projectPath: string, startDate: Date, endDate: Date) {
  const git = simpleGit(projectPath);
...
function generateProductivityInsights(trends: any[]): string[] {
